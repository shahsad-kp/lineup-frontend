...
export default function SetupLayout() {
    ...
    const [connectedCalendarAccounts, setConnectedCalendarAccounts] = useState<CalendarAccount[]>([]);
    const [openedCalendarAccount, setOpenedCalendarAccount] = useState<CalendarAccount | null>(null);
    const [showAddAccountModal, setShowAddAccountModal] = useState<boolean>(false);
    const router = useRouter();

    const onCloseModal = useCallback(() => {
        if (openedCalendarAccount) {
            updateConnectedAccount(openedCalendarAccount.id, accountName).then(calendar => {
                setConnectedCalendarAccounts((prev) => {
    ...
    }, [accountName, openedCalendarAccount]);

    const openAddAccountModal = useCallback((calendar: CalendarAccount | null = null) => {
        if (calendar) {
            setOpenedCalendarAccount(calendar);
    ...
    const connectMicrosoftAccount = useCallback(() => {
        const REDIRECT_URI = `${process.env.NEXT_PUBLIC_DOMAIN}/auth/callback/`;
        ...
        const authUrl = `https://login.microsoftonline.com/common/oauth2/v2.0/authorize?${authUrlQuery.toString()}`;
    ...
    return (
        <>
            <div>
                <SetupTracker totalSteps={4} currentStep={0}/>
                <div className={'w-full flex flex-col gap-3 mt-10'}>
                    <div className={'w-full flex flex-row gap-2'}>
                        <h4 className={'text-text-secondary font-medium'}>Connect your calendar accounts</h4>
                        <Tooltip title="Connecting your calendar accounts will allow you to sync your events and schedules
                                    with our platform." variant="outlined">
                    ...
                    <Stack spacing={1}>
                        {
                            connectedCalendarAccounts.map(calendar => {
                                return (
                                    <Button
                                        key={calendar.id}
                                        sx={{
                                        ...
                                        variant="soft"
                                        onClick={() => openAddAccountModal(calendar)}
                                    >
                                        {
                                        ...
                                        <Typography level={'body-md'} noWrap={true}>{calendar.name}</Typography>
                                    </Button>
                                )
                            })
                        }
                        ...
                        >
                            <Image src={'/icons/plus-icon.svg'} alt={'Plus Icon'} className={'w-5 h-5'} width={20}
                                   height={20}/>
                            <Typography level={'body-md'} noWrap={true}>Add a calendar account</Typography>
                        </Button>
                    </Stack>
                    <div className={'w-full flex flex-row justify-end'}>
                        <Button
                            disabled={connectedCalendarAccounts.length === 0} variant="solid"
                            onClick={() => router.push('/setup/event-calendar')}
                        >
                            Next
                        </Button>
                    </div>
            ...
            {/*<Modal
                    ...
                    //                         {
                    //                             openedCalendarAccount.provider === 'Google' ? (
                    //                                 <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 200 200" width="24"
                    //                                      height="24">
                    //                                     <path
                    //                                         d="M152.632 47.368l-47.368-5.263-57.895 5.263L42.105 100l5.263 52.632L100 159.211l52.632-6.579 5.263-53.947z"
                    ...
                    //                             )
                    //                         }
                    //                         <span>{openedCalendarAccount.name}</span>
                    //                     </div>
                    //                 )}
                    //             </div>
                    //             <div className={'mt-3'}>
                    ...
                    //             <Button variant="solid" onClick={() => closeModal()}>Close</Button>
                    //         </div>
                    //     </div>
                    // </div>
 */}
